...
> <strong>Choose</strong> which one to use, then make sure your system's <tt>$PATH</tt> variable includes <strong>just one</strong> of the above paths (and only one, not both).

   <ul><pre><strong>echo $PATH</strong></pre></ul>

Developers using Mac Books but NOT developing apps to run on an <a href="#ApplePlatforms">Apple platform</a> can <a href="#InstallCU">install just the CommandLineTools</a>.
...
Build version 14E300c
   ...
   This should be the same as the version displayed on Apple's web page</a>.
...
BTW Increasingly, Machine Learning developers are using Apple's Swift language.
