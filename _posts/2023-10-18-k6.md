...
It was written in Go within loadimpact.com and in May 2020 was acquired by Grafana Labs, which makes dashboards and Prometheus. 

Unlike JMeter and LoadRunner, k6 FOSS is a command-line tool to run performance (load) tests on local machines.
...
1. Alternately, override the init code options with parameters in the run command, such as:

   <pre><strong>time k6 run --vus 5 --duration 10s k6-test-ping.js
   </strong></pre>

1. Get to know the various parameters:
1. Get the k6 menu:
