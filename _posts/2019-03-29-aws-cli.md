...
2. Verify it works by running a sample command:
   ...
   The response should be: <tt>"b"</tt> from "[1]".

   <a target="_blank" href="https://github.com/jmespath/jp/issues/17">WHOOPS</a>: 
   <tt>line.go:44: no valid y values given</tt>
...
7. Run the command to create files in folder ~/aws referenced by all other aws cli commands:
   ...
   <pre><strong>aws configure --profile <em>acct name</em>
   </strong></pre>
