...
AWS DynamoDB provides fault tolerance in automatic synchronous replication across 3 data centers in a region.

DynamoDB streams replicate table activity across geographic regions.

Being a cloud service, it automatically allocates storage in partitions and imposes no limit to data storage.

Its SELECT operations are like SQL but not exactly.<br />
So it's for ports of apps from SQL relational databases that have joins.

Use S3 for storing blob data > 400 KB.

It's managed via a REST API.
...
Amazon QuickSight is an AWS-managed SaaS interactive visual dashboard for displaying results from BI (Business Intelligence) queries. 
...
AWS Data Exchange to create a data mesh or meet other data sharing needs with no data movement.
 using 
Services by Lake Formation include:
