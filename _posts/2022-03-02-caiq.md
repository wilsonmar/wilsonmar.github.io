...
15. <a name="AIS-05.2">AIS-05.2 - Automated Application Security Testing</a>
    ...
    <a name="AIS-06-M1">CCM AIS-06-M1</a> METRIC = Percent of running production code that can be directly traced back to automated security and quality tests that verify the compliance of each build. (SLO 95%)
...
27. <a name="BCR-05.3">BCR-05.3 - Documentation</a>
    ...
    <a name="BCR-06-M1">CCM BCR-06-M1</a> METRIC = Percent of critical systems that passed Business Continuity Management and Operational Resilience (CCMv4 domain BCR) tests. (SLO 80%)
...
40. <a name="CCC-02.1">CCC-02.1 - Quality Testing</a>
    ...
    <a name="CCC-03-M1">CCM CCC-03-M1</a> METRIC = Percent of all assets that have change management technology integrated. (SLO 80%)
...
44. <a name="CCC-06.1">CCC-06.1 - Change Management Baseline</a>
    ...
    <a name="CCC-07-M1">CCM CCC-07-M1</a> METRIC = Percent of positive test results from all configuration tests performed. (SLO 95%)
...
51. <a name="CEK-02.1">CEK-02.1 - CEK Roles and Responsibilities</a>
    ...
    <a name="CEK-03-M2">CCM CEK-03-M2</a> METRIC = Percent of cryptographic module continues to be up to approved standards. (SLO 85%)
...
52. <a name="CEK-03.1">CEK-03.1 - Data Encryption</a>
    ...
    <a name="CEK-04-M1">CCM CEK-04-M1</a> METRIC = Percent of assets with cryptographic functions that meet an organizationÕs defined cryptographic requirements. (SLO 90%)
...
82. <a name="DCS-05.1">DCS-05.1 - Assets Classification</a>
    ...
    <a name="DCS-06-M1">CCM DCS-06-M1</a> METRIC = ratio of managed assets (i.e., cataloged and tracked) to detected assets. (SLO 95%)
...
98. <a name="DSP-03.1">DSP-03.1 - Data Inventory</a>
    ...
    <a name="DSP-04-M3">CCM DSP-04-M3</a> METRIC = ratio of assets in the asset catalog that have been classified according to the data classification policies specific to each organization. (SLO 99%)
...
122. <a name="GRC-03.1">GRC-03.1 - Organizational Policy Reviews</a>
     ...
     <a name="GRC-04-M1">CCM GRC-04-M1</a> METRIC = Percent active policy exceptions where the time to resolution is within the documented timeline for resolution. (the effectiveness of governance exception handling processes) (SLO 90%)
...
155. <a name="IAM-06.1">IAM-06.1 - User Access Provisioning</a>
     ...
     <a name="IAM-07-M1">CCM IAM-07-M1</a> METRIC = Percent of users leaving the organization that were deprovisioned from the identity management system in compliance with identity and access management policies. (SLO 99%)
...
156. <a name="IAM-07.1">IAM-07.1 - User Access Changes and Revocation</a>
     ...
     <a name="IAM-08-M2">CCM IAM-08-M2</a> METRIC = ??? Percent of time elapsed since the last recertification for all types of privileges (including user roles, group memberships, read/write/ execute permissions to files/databases/scripts/jobs, etc). (SLO 95%)
...
157. <a name="IAM-08.1">IAM-08.1 - User Access Review</a>
     ...
     <a name="IAM-09-M1">CCM IAM-09-M1</a> METRIC = Percent of users with production access having admin access (segregation of duties) (SLO 99%)
...
174. <a name="IPY-02.1">IPY-02.1 - Application Interface Availability</a>
     ...
     <a name="IPY-03-M2">CCM IPY-03-M2</a> METRIC = Percent of data flows that use an approved, standardized cryptographic security function for interoperable transmissions of data. (SLO 99.99%)
...
184. <a name="IVS-03.5">IVS-03.5 - Network Security</a>
     ...
     <a name="IVS-04-M1">CCM IVS-04-M1</a> METRIC = Percent of assets in compliance with the providerÕs configuration security policy and hardening baselines derived from accepted industry sources (e.g., NIST, vendor recommendations, Center for Internet Security Benchmarks, etc.). (SLO 99.99%)
...
193. <a name="LOG-02.1">LOG-02.1 - Audit Logs Protection</a>
     ...
     <a name="LOG-03-M1">CCM LOG-03-M1</a> METRIC = Percent of logs configured to generate security alerts for anomalous activity across control domains (SLO 95%)
...
196. <a name="LOG-04.1">LOG-04.1 - Audit Logs Access and Accountability</a>
     ...
     <a name="LOG-05-M1">CCM LOG-05-M1</a> METRIC = Percent of discovered anomalies resolved within required timelines  (effectiveness of the log monitoring and response process) (SLO 95%)
...
203. <a name="LOG-09.1">LOG-09.1 - Log Protection</a>
     ...
     <a name="LOG-10-M1">CCM LOG-10-M1</a> METRIC = Percent of cryptography, encryption, and key management controls with defined metrics. (SLO 80%)
...
206. <a name="LOG-12.1">LOG-12.1 - Access Control Logs</a>
     ...
     <a name="LOG-13-M2">CCM LOG-13-M2</a> METRIC = Percent  uptime minutes of the monitoring system during the sampling period (SLO 99%)
...
214. <a name="SEF-04.1">SEF-04.1 - Incident Response Testing</a>
     ...
     <a name="SEF-05-M1">CCM SEF-05-M1</a> METRIC = Percent of security events sourced from automated systems. (SLO 90%)
...
226. <a name="STA-06.1">STA-06.1 - SSRM Control Implementation</a>
     ...
     <a name="STA-07-M5">CCM STA-07-M5</a> METRIC = Percent of approved supply chain upstream cloud services relationships that are not recorded in logged data connections. (SLO 99%)
...
238. <a name="TVM-02.2">TVM-02.2 - Malware Protection Policy and Procedures</a>
     ...
     <a name="TVM-03-M1">CCM TVM-03-M1</a> METRIC = Percent of high and critical vulnerabilities that are remediated within the organizationÕs policy timeframes. This reflects the time between when a vulnerability is identified on an organizationÕs assets and when remediation is complete. (SLO 99%)
...
242. <a name="TVM-06.1">TVM-06.1 - Penetration Testing</a>
     ...
     <a name="TVM-07-M1">CCM TVM-07-M1</a> METRIC = Percent of managed assets scanned monthly. (SLO 99%)
...
245. <a name="TVM-09.1">TVM-09.1 - Vulnerability Management Reporting</a>
     ...
     <a name="TVM-10-M1">CCM TVM-10-M1</a> METRIC = Percent of publicly known vulnerabilities that are identified for an organizationÕs assets within the organizationÕs required timeframes. (SLO nan)
...
250. <a name="UEM-03.1">UEM-03.1 - Compatibility</a>
     ...
     <a name="UEM-04-M1">CCM UEM-04-M1</a> METRIC = Percent of endpoints actively maintained in the asset inventory (appearing in security audit logs) (SLO 95%)
...
251. <a name="UEM-04.1">UEM-04.1 - Endpoint Inventory</a>
     ...
     <a name="UEM-05-M1">CCM UEM-05-M1</a> METRIC = Percent of unique endpoints with suitable policy enforcement tools that have reported compliance state within the sampling period (ability of an organization to control the configuration and behavior of assets which directly create, read, write, or delete organizational data) (SLO 99%)
...
255. <a name="UEM-08.1">UEM-08.1 - Storage Encryption</a>
     ...
     <a name="UEM-09-M1">CCM UEM-09-M1</a> METRIC = Percent of instances which are an running anti-malware/virus service. (SLO 99%)
