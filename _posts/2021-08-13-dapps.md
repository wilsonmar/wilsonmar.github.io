...
Governments are naturally resistant to decentralized mechanisms because governments are in the business of centralized control, with the ability to shutdown Facebook, Instagram, Twitter, etc. Decentralized systems are difficult to shut down. Wikipedia has not been accessible in Turkey for a few years.
...
Blockchains are not designed to store files. And current file storage mechanisms such as NFS, Google Drive, Dropbox, etc. are central Web 2.0 technologies.
...
A file such as https://en.wikipedia.org/wiki/Aardvark, on a website owned by Wikipedia, who can grant or deny access.

In IPFS that file would be retrieved using this CID (Content Identifier), a unique fingerprint:
...
It is the third major blockchain innovation:
   ...
   2. Etherium smart contracts, which made programmable to power the "DiFi" (Distributed Finance) revolution
   3. Infinite blockchain with seamless capacity of special nodes run by independent data centers
   <br /><br />

The "Internet Computer" hosts "canisters" (smart contracts in the Internet Computer blockchain) 
...
1. Deploy to the Internet Computer ledger (instead of to web servers):

   <pre><strong>def deploy --network=ic
   </strong></pre>
