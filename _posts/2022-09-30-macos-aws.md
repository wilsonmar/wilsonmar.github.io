...
### AWS Dedicated Hosts Availability
...
21. Pull down the <strong>Actions</strong> list to select "Launch instance(s) onto host" for the "Launch an instance" form.

22. Specify a Name with your team name, Availability Zone, mac1/2, etc.

    <tt>wilson-us-east-1a-mac2</tt>

23. Select the "macOS" icon for a list of instance types for the region. Example for Oregon:
...
24. If you don't want the latest MacOS operating system (Ventura at time of this writing), click the down arrow icon to select the previus macOS release (such as Monterey).
    
25. Click the selection under "Architecture" and select "64-bit (Mac-Arm)" for Mac2.
   
26. In the list that appears, click the checkbox on the line 
...
27. For Instance type, click "Select", then type "mac2" for "mac2.metal".

    PROBLEM: I can seem to click "mac2.metal"


   ### Create new key pair

8. For <strong>Key pair name</strong>, click "Create new key pair" if you didn't create one before this.
...
10. Create a folder where you will store access credentials, such as <tt>~/projects/macos-aws</tt>

11. PROTIP: <a target="_blank" href="https://www.youtube.com/watch?v=8UqtMcX_kg0">As with other instance types</a>, define a Security Group using port 22 protocol TCP source <strong>your laptop's IP address</strong> (rather than 0.0.0.0/0 for just anyone, which is unsafe).
12. Create a pem key (such as "malx-us-east-1a.pem" in the example below) and save it to the folder.

13. In Terminal: <tt>chmod 0400 malx-us-east-1a.pem</tt>
14. Define an IAM role. ???

15. Click the orange "Launch instance".
...
6. At the top upper-right, select a Region ("us-east-1", us-east-1, or us-east-2) that supports <a href="#AvailableMetal">a mac*.metal</a> among its Availability Zones.
...
30. For Key pair (login), click "Create new key pair" if you haven't already created a pem file. 
31. Create security group to Allow SSH traffic from "My IP".
32. In Advanced details, you cannot check "Request Spot instances".
33. Under "Tenancy", select "Dedicated host - launch this instance on a dedicated Host".
...
34. Click the "Next: Storage" to
35. increase the size of the Root Volume from a default of 60 to 300 GiB (or whatever your capacity analysis runs reveal). 
36. Select Volume Type:
...
37. Add tags per your administrator's guidelines and examples.
...
38. Click the blue "Launch" for "Success! Successfully initiated launch of instance".
39. Click "Instances" and scroll down to your Instance ID.
40. Press command+R to refresh until "Status check: Initializing" changes to "passed". ???

   CAUTION: Do not click "Launch instances" in the Instances UI.

41. Return to "Dedicated Hosts".

   ### Connect

42. Click "Connect to instance".
43. Click the orange "Connect" which creates a new browser window.
...
44. After Instance State is "Running",
45. Highlight the Public IPv4 address to your Clipboard.
46. Proceed to <a href="#AWSMacConnect">Connect to AWS MacOS</a> below.
<br /><br />
...
Avoid paying for cycles you don't use by setting up either
a calendar entry to manually shut down the instance
or schedule an automated action.
