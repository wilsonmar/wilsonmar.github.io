...
Having a mac in the AWS cloud has these advantages:
   ...
   * Each mac can expand the size and type of disk storage space it has access to (EBS IOPS, S3, etc.).
...
5. Remotely retrieve and then run <a target="_blank" href="https://github.com/wilsonmar/mac-setup/blob/master/aws-info.sh">my shell script</a> to list the mac*.metal instances by copying and pasting this command on your Terminal:

    <pre><strong>
    bash -c "$(curl -fsSL https://raw.githubusercontent.com/wilsonmar/mac-setup/master/aws-info.sh)" -v -ec2info
    </strong></pre>
...
1.  In the AWS Console Home, select a region such as "Oregon" (us-west-2) because a different region changes the Quata Code within the destination URL after following steps below:
    ...
    1. At the top of the Console Home Search bar, type <strong>limits</strong>.
    2. Select "Service Quotas", "AWS Services".
    3. Within "Find services", type "EC2" and click "Amazon Elastic Compute Cloud (Amazon EC2)".
    4. Within "Find quotas", type "mac2" and click "Running dedicated mac2 hosts".
    <br />
    ...
    The "Applied quota value" needs to be non-zero, such as "5".
...
2.  Click the AWS logo at the upper-left corner. 
3.  Select a region such as "Oregon" (us-west-2).
4.  Click <tt>EC2</tt> or type EC2 in the Search bar.
5. In the EC2 menu select <strong>Dedicated Hosts</strong> or type that in the Search field and select it among Features returned.
6. Click the orange "Allocate Dedicated Host" at the right.
7. Construct a Name tag (such as "johndoe-mac2a").

   PROTIP: Use the name from your email so people know how to find you.

8.  Click "Choose Instance family" and <strong>scroll</strong> to select <a href="#mac1or2">"mac2"</a>.

    <a name="NoMultipleInstances"></a>

9.  <a target="_blank" href="https://medium.com/aws-architech/the-instance-family-mac1-is-not-supported-b6ca31154f22">IMPORTANT</a>: Uncheck default "Support multiple instance types" to avoid getting</a> the cryptic error message:

    <tt>The instance family 'mac2' is not supported.</tt>

10. Click "Choose Instance type" and select "mac2.metal".
11. Click "Choose Availability Zone" and select <a href="#AvailableMetal">the one which appeared in your search above</a>, such as "us-west-2d".
12. For "Outpost ARN", leave alone.

13. Leave "Instance auto-placement" <strong>unhecked</strong>, the default, which preserves the Dedicated Host for targeted launch requests. Enabling auto-placement will allow untargeted (no host ID specified) launch requests to succeed.
14. Leave "Host recoveryInfo" <strong>unchecked</strong>, the default. It restarts your instances on to a new replacement host if failures are detected on your Dedicated Host. 
15. Leave "Quantity" to 1.

    <a name="installbrews"></a>

16. Click "> CLI commands" to add what you want executed upon on the instance's Terminal after boot-up:
...
17. Specify a Tag for accounting and budgeting, etc.
...
18. Click the orange "Allocate". 

    PROTIP: If you get this message, read <a href="#NoMultipleInstances">this (above)</a>:

    <tt>The instance family 'mac2' is not supported.</tt>

19. PROTIP: Highlight and copy your Host ID (such as "h-04c3a0f681de175c8") and Availability Zone (such as "us-west-2a") to a note file so your remember.

    WARNING: "Sucessfully created host h-???" means that charges have begun to accumulate (in hourly increments), even though you do not use the instance.

20. Normally, you would <a href="#CreateEC2instance">create an EC2 instance to use the Dedicated Host</a> you just created.

PROTIP: If you are not going to <a href="#ConfigLaunchInstance">Configure Launch Instance</a> (make use of) the Dedicated Host <strong>within the hour</strong>, please release it.
...
1. In the list of Dedicated Hosts on the EC2 menu:

    https://us-west-2.console.aws.amazon.com/ec2/home?region=us-west-2#Hosts:
 
2.  Click the checkbox associated with the (Dedicated) Host ID you created.

3.  Pull down the <strong>Actions</strong> list to select "Launch instance(s) onto host" for the "Launch an instance" form.

4.  Specify a Name with your team name, Availability Zone, mac1/2, etc.

    <tt>johndoe-us-west-2d-mac2-001</tt>

    <tt>-001</tt> is if you're bringing up several instances for a multi-machine test.

5.  Select the "macOS" icon for a list of instance types for the region. Example for Oregon:
...
6.  If you don't want the latest MacOS operating system (Ventura at time of this writing), click the down arrow icon to select the previus macOS release (such as Monterey).
    
7.  Click the selection under "Architecture" and select "64-bit (Mac-Arm)" to override the default which is incorrect for mac2.
...
9.  Construct a <strong>Key pair name</strong> that distinguishes it from other key pairs, such as this, to enable you to reuse it:
...
10. PROTIP: Highlight and copy (using Command+C) the value to your computer's hidden Clipboard to paste in your work notes file.
11. For key pair type, keep RSA the default (not ED25519).
12. For Private key file format, click ".pem" for macOS.
13. Click the orange "Create key pair" for the <strong>private key</strong> to be downloaded to your <strong>Downloads folder</strong>.
    
    WARNING: Do not move the .pem secret file to a folder (away from your $HOME folder) where it may be uploaded publicly.

    ### Set permissions

    This is not necessary at this point, but while it's fresh on your mind:

14. Switch to your Terminal to set the .pem file's permissions:

    <pre><strong>chmod 600 johndoe-rsa-pem-231231
    </strong></pre>

    This prevents an error when SSH issues it.

    ### Network settings - Security Group

15. If you have created a Security Group for your IP address, select "Select existing security group" and scroll to pick it.
    
    Otherwise: 
    1. Select "Create security group" 
    2. Check <strong>Allow SSH traffic from</strong> 
    3. Select "My IP" for better security of restricting traffic to only the IP you use.
    <br >
...
18. Click the arrow in "> Configure storage".
19. Notice that, by default, the instance get 100 GiB of "gp2" (General Purpose SSD) for its Root volume used for boot-up.
...
20. Click "Advanced" at the right to configure EBS Volumes.
21. increase the size of the Root Volume from a default of 60 to 300 GiB (or whatever your capacity analysis runs reveal). 
22. Select Volume Type:
...
23. Add tags per your administrator's guidelines and examples.
...
24. Click the arrow in front of "Advanced Details".
25. For "Shutdown behavior", click "Stop" to select "Terminate" if you are experimenting and want a fresh start every time.

26. Leave these settings to their default unless you'd like to overrride.

    Under "Tenancy", select "Dedicated host - launch this instance on a dedicated Host".
...
26. Scroll to the bottom among "Advanced details".
27. If you have commands to be executed upon on the instance's Terminal after boot-up:
...
27. Click the orange "Launch instance" for a message such as:
    
    Success! Successfully initiated launch of instance (i-1234567890abcdef)

28. Optionally, click "Launch log".
29. Click the orange "View all instances"
    
    https://us-west-2.console.aws.amazon.com/ec2/home?region=us-west-2#Instances:

30. Click the blue Instance ID for the "mac2.metal" instance you just created. Alternately, select the checkbox for your instance andcClick "Actions" to select View Details.
31. Status check will say "Initializing" initially.
32. Note the Public IPv4 address, such as <tt>44.234.9.188</tt> which will be different when another instance is created.

33. Press command+R to refresh until "Status check: Initializing" changes to green "Running".

    <a name="ConnectUsingGUI"></a>

    ### Connect using GUI

37. After Instance State is "Running"
38. Notice that "User name" is <tt>ec2-user</tt>
39. Click the "Connect" box.

    Do not click the orange "Connect" with the default "EC2 Instance Connect" selected. That creates a new browser window. You will see this in red:
...
40. Click "SSH client" for instructions to appear.

    PROTIP: Instructions below are enhanced version of what AWS presents.

41. Open an SSH client, such as your macOS Terminal.
42. Navigate to a working folder, which you'll be using to upload and download files for your EC2 instances.
43. Define the pem file's location in a variable so you don't have to retype it:

    <pre><strong>export PEM_FILE="$HOME/Downloads/wilsonmar-rsa-pem-231231.pem"
    </strong></pre>

44. Verify:

    <pre><strong>echo "$PEM_FILE"
    ls -al
    </strong></pre>

45. Set permissions needed:

    <pre><strong>chmod 400 "$PEM_FILE"
    </strong></pre>

46. Click the icon next to the Public DNS, such as:

    <tt>ec2-44-234-9-188.us-west-2.compute.amazonaws.com</tt>

47. Construct the command to save the DNS in a variable:

    <pre><strong>export PUBLIC_DNS=ec2-44-234-9-188.us-west-2.compute.amazonaws.com
    </strong></pre>

48. Verify:

    <pre><strong>echo "$PUBLIC_DNS"
    </strong></pre>

49. Highlight this command and paste in your Terminal to connect SSH:

    <pre><strong>ssh -i "$PEM_FILE" "ec2-user@$PUBLIC_DNS"
    </strong></pre>

49. It takes a few seconds to connect.
50. Highlight the Public IPv4 address to your Clipboard.
51. Proceed to <a href="#AWSMacConnect">Connect to AWS MacOS</a> below.
<br /><br />


### Terminate EC2 Instance

1. In the list of Dedicated Hosts that appears:

    https://us-west-2.console.aws.amazon.com/ec2/home?region=us-west-2#Hosts:
 
2.  click the checkbox associated with the Host ID you created.
3.  <a target="_blank" href="https://www.youtube.com/watch?v=dapXcfhMoGw">VIDEO</a>: Use a browser to the EC2 Instances page for your region.
    
    https://us-west-2.console.aws.amazon.com/ec2/home?region=us-west-2#Instances

4.  Check a "mac2.metal" Instance Type.
5.  Pull down the "Instance state" menu to select "Terminate instance".
6.  "Shutting-down" will continue to appear after you refresh the screen after "Successfully terminated" alert appears at the top.
7.  The instance should eventually disappear from the list, leaving "No matching instances found".
...
1. <a target="_blank" href="https://www.youtube.com/watch?v=8UqtMcX_kg0">As with other instance types</a>, use SSH to connect to your Mac instance :
   ...
   SSH_TARGET can be username@<em>publicDNS</em> or <em>IP Address</em>

   <pre><strong>KEY_PAIR="malx-us-west-2.pem"
   SSH_TARGET="ec2-user@35.161.43.6"
   yes | ssh -i "$KEY_PAIR" "$SSH_TARGET"
   </strong></pre>

   The <tt>yes | </tt> is to automatically answer<br />
   <pre>Are you sure you want to continue connecting (yes/no)?
   </pre>


2. Once on the Mac prompt:
   ...
   NOTE: Your password would be required.

3. Disconnect and reconnect to enable the GUI:

   <pre><strong>ssh -L 5900:localhost:5900 \
   -i "$KEY_PAIR"  "$SSH_TARGET"
   </strong></pre>

   You should now be seeing the mac screen.

   ### Actions inside macOS

4. Press command+spacebar for Spotlight to search for the screen-sharing app.
...
5. Login using your password.
   ...
   Remotely controlling another computer makes use of the Remote Frame Buffer protocol (RFB) to relay graphical-screen updates and transmit keyboard and mouse input from one computer to another over a network.

   macOS does not officially support Microsofts’ Remote Desktop Protocol, which is “semantic” (it’s aware of controls, fonts, etc. such that controls are drawn by the client application),  thus sending much less data to the client application and a much cleaner display of the screen of the controlled computer.
...
6. To enable the VNC <strong>server</strong>, enter the macOS "System Preferences" by clicking the Apple icon in the upper left corner. 
7. Click the blue "Sharing" icon or type enough of "Sharing" in the Search box at the upper-right to choose it.

   <a target="_blank" href="https://res.cloudinary.com/dcajqrroq/image/upload/v1674375940/macos-sharing-1288x606_t78qaf.jpg"><img alt="macos-sharing-1288x606.jpg" src="https://res.cloudinary.com/dcajqrroq/image/upload/v1674375940/macos-sharing-1288x606_t78qaf.jpg"></a>

8.  Click "Computer Settings..."
9.  Check "VNC viewers may control screen with password:"

10. Create a password of sufficient complexity. 8 characters is all that Apple allows.
11. Store that password in a secrets vault so you can't forget it.
12. Type that password in the form. Press OK.
   
    Alternately, issue a CLI command (via SSH) to enable Sharing after defining VNC_PASSWORD:
    ...
    Instead of using a single VNC password, to share access with any and all (which is less secure and not recommended), replace the last two lines above with:
...
13. On the client computer that controls the VNC server computer remotely, install the <a target="_blank" href="https://www.realvnc.com/en/connect/download/viewer/macos/">Real VNC <strong>viewer</strong></a>:

    <pre><strong>brew install --cask vnc-viewer
    open "$HOME/Applications/VNC Viewer.app"
    </strong></pre>

14. Click "Open" to the pop-up download alert.
15. Check to accept its terms, then click OK.
16. Provide your email and Password "that you used to create your RealVNC account".
