...
<pre><strong>export AWS_REGION="us-east-1"
export AWS_AZ="us-east-1a"
export AWS_INSTANCE_TYPE="mac2.metal"
aws ec2 describe-instance-type-offerings \
   --region "${AWS_REGION}" \
   --location-type "availability-zone" \
   --output text \
   --filters "Name=location,Values=${AWS_AZ}"
   --filters "Name=InstanceType,Values=m5.metal"
   </strong></pre>

# With no --query --output text  | wc -l :
   # 510

# With no --query --output json:
# {
...
# For multiple filters, include a space between different filters:
#   --filters "Name=location,Values=${AWS_AZ}" \   is known good
#   --filters Name=InstanceType,Values="${AWS_INSTANCE_TYPE}" \
#   --filters "Name=instancetype,Values=m5.metal" \
# m5.metal 

# With --query "InstanceTypeOfferings[*]" to remove repetitive first level:
# With --output text  :
   # INSTANCETYPEOFFERINGS   r6a.32xlarge    us-east-1a      availability-zone
   # INSTANCETYPEOFFERINGS   r6gd.2xlarge    us-east-1a      availability-zone
#   --query "InstanceTypeOfferings[*].Location,InstanceTypeOfferings[*].InstanceType," \


   --query "InstanceTypeOfferings[*].[InstanceType]" \
   --output text | sort

   <tt>--query</tt> is a JMESPath query to use in filtering the response data.

   References about JMESPath:
   * https://stackoverflow.com/questions/43532308/how-to-filter-results-from-aws-cli-command
   ...
   * https://opensourceconnections.com/blog/2015/07/27/advanced-aws-cli-jmespath-query/ on aws ec2 describe-images
   <br /><br />
