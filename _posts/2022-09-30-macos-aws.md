...
The ways to get a mac*.metal instance:
   ...
   >>> <a name="OptionA"><strong>Option A: Use AWS Management Console GUI manually</strong></a> (not preferred as it's not repeatable, but good for learning)
   <br /><br />
   >>> <a name="OptionB"><strong>Option B: Use CLI commands locally on your laptop</strong></a> (programmatic commands that require testing adequacy of conditions and verification of results)
   <br /><br />
   >>> <a name="OptionC"><strong>Option C: Use Terraform</strong></a> (declarative JSON that's simpler and repeatable)
...
9.  Construct a <strong>Key pair name</strong> that distinguishes it from other key pairs, such as:

    <tt>wilsonmar-us-east-1a-rsa-231231</tt>
...
10. PROTIP: Highlight and copy (using Command+C) the value to your computer's hidden Clipboard.
10. For key pair type, keep RSA the default (not ED25519).
11. For Private key file format, click ".pem" for macOS.
12. Click the orange "Create key pair" for the <strong>private key</strong> to be downloaded to your <strong>Downloads folder</strong>.
    
    WARNING: Do not move the .pem secret file to a folder (away from your $HOME folder) where it may be uploaded publicly.

21. Switch to your Terminal to set the .pem file's permissions:

    <pre><strong>chmod 600 wilsonmar-us-east-1a-rsa-231231
    </strong></pre>
...
22. Select "Create security group" 
23. Check <strong>Allow SSH traffic from</strong> 
24. Select "My IP" for better security of restricting traffic to only the IP you use.
    
    ### Boot-up commands

25. Click the arrow in front of "Advanced details" to expand it.
26. You cannot check "Request Spot instances" for macOS.
...
28. PROTIP: <a target="_blank" href="https://www.youtube.com/watch?v=8UqtMcX_kg0">As with other instance types</a>, define a Security Group using port 22 protocol TCP source <strong>your laptop's IP address</strong> (rather than 0.0.0.0/0 for just anyone, which is unsafe).
29. Define an IAM role. ???

30. Check "Allow SSH traffic from": Anywhere.
...
31. Click the "Next: Storage" to:
32. increase the size of the Root Volume from a default of 60 to 300 GiB (or whatever your capacity analysis runs reveal). 
33. Select Volume Type:
...
34. Add tags per your administrator's guidelines and examples.
...
35. Leave these settings to their default unless you'd like to overrride.

    ### Launch

36. Click the blue "Launch" for "Success! Successfully initiated launch of instance".
37. Click "Instances" and scroll down to your Instance ID.
38. Press command+R to refresh until "Status check: Initializing" changes to "passed". ???

    CAUTION: Do not click "Launch instances" in the Instances UI.

39. Return to "Dedicated Hosts".
