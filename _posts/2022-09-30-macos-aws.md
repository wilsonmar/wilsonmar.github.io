...
1.  In the AWS Console Home, select a region such as "Oregon" (us-west-2) because a different region changes the Quata Code within the destination URL after following steps below:
    ...
    1. At the top of the Console Home Search bar, type <strong>limits</strong>. This applies to all AWS services.
    2. Select "<a target="_blank" href="https://docs.aws.amazon.com/servicequotas/latest/userguide/intro.html">Service Quotas</a>", then "AWS Services" on the left.
    3. Within "Find services" search, type "EC2" and click "Amazon Elastic Compute Cloud (Amazon EC2)".
    4. Within "Find quotas", type "mac" and click "Running dedicated mac2 hosts".
    <br />
    ...
    The "Applied quota value" needs to be non-zero, such as "5" shown above, in order to enable creation of EC2 Dedicated Hosts.

    TODO: The <a target="_blank" href="https://docs.aws.amazon.com/cli/latest/reference/service-quotas/">CLI command</a> for the above is:

    <pre><strong>AWS_QUOTA_CODE=L-5D8DADF5
...
4.  Click <tt>EC2</tt> among Recently visited or type EC2 in the Search bar.
5. In the EC2 left menu select <strong>Dedicated Hosts</strong> or type that in the Search field and select it among Features returned.
6. Click the orange "Allocate Dedicated Host" at the right.
7. Construct a Name tag. PROTIP: Include in the name your email so people know how to find you. Example:
   
   <strong>johndoe-us-west-2d-mac2-001</strong>   

8.  Click "Choose instance family" and type <strong>mac</strong> (even though there is no feedback) to select <a href="#mac1or2">"mac2"</a>.

    <a name="NoMultipleInstances"></a>

9.  <a target="_blank" href="https://medium.com/aws-architech/the-instance-family-mac1-is-not-supported-b6ca31154f22">IMPORTANT</a>: Uncheck <strong>Enable</strong> default "Support multiple instance types" to avoid getting</a> the cryptic error message:

    <tt>The instance family 'mac2' is not supported.</tt>

10. Click "Choose Instance type" and select "mac1.metal" or "mac2.metal".
...
19. PROTIP: Highlight and copy your Host ID (such as "h-04c3a0f681de175c8") to paste in the note file where you stored the Availability Zone (such as "us-west-2d").

20. Click the Host ID in blue for its "Dedicated Hosts details" page.
...
21. Normally, you would proceed to <a href="#CreateEC2instance">create an EC2 instance to use the Dedicated Host</a> you just created.

PROTIP: If you are not going to immediately <a href="#ConfigLaunchInstance">Configure Launch Instance</a> (make use of) the Dedicated Host <strong>within the hour</strong>, please <a href="#ReleaseHost">release it</a> so it's not eating up money unproductively.
...
1.  If you are at the EC2 list of Dedicated Hosts, click the checkbox associated with the (Dedicated) Host ID you created.

    https://us-west-2.console.aws.amazon.com/ec2/home?region=us-west-2#Hosts:
    
    Alternately, if you're already at the "Dedicated Hosts details" page of a particular Host:

2.  Pull down the <strong>Actions</strong> list to select "Launch instance(s) onto host" for the "Launch an instance" form.

3.  Construct a Name with your own name, Availability Zone, mac1/2, etc.

    <tt>johndoe-us-west-2d-mac2-001</tt>

    <tt>-001</tt> is if you're bringing up several instances for a multi-machine test.

4.  Select the "macOS" icon for a list of instance types for the region. Example for Oregon:
...
5.  If you don't want the latest MacOS operating system (Ventura at time of this writing), click the down arrow icon to select the previus macOS release (such as Monterey).
    
6.  Click the selection under "Architecture" and select "64-bit (Mac-Arm)" to override the default which is incorrect for mac2.
...
7.  For <strong>Key pair name</strong>, click "Create new key pair".
...
8.  Construct a <strong>Key pair name</strong> that distinguishes it from other key pairs, such as this, to enable you to reuse it:
...
9.  PROTIP: Highlight and copy (using Command+C) the value to your computer's hidden Clipboard to paste in your work notes file.
...
14. If you have already created a Security Group for your IP address, select "Select existing security group" and scroll to pick it.
...
15. Write down the name of the Security Group created because they are <strong>not deleted automatically</strong>. Thus, a process is needed to <a href="#RemoveSGroups">identify and remove abandoned Security Groups no longer used</a>.
...
16. Click the arrow in "> Configure storage".
17. Notice that, by default, the instance get 100 GiB of "gp2" (General Purpose SSD) for its Root volume used for boot-up.
...
18. Click "Advanced" at the right to configure EBS Volumes.
19. increase the size of the Root Volume from a default of 60 to 300 GiB (or whatever your capacity analysis runs reveal). 
20. Select Volume Type:
...
21. Add tags per your administrator's guidelines and examples.
...
22. Click the arrow in front of "Advanced Details".
23. For "Shutdown behavior", click "Stop" to select "Terminate" if you are experimenting and want a fresh start every time.

24. Leave these settings to their default unless you'd like to overrride.
...
25. For "Target host by", select the ID of the Dedicated Host you created (above).
...
28. Click the orange "Launch instance" for a message such as:
    
    Success! Successfully initiated launch of instance (i-1234567890abcdef)

29. Optionally, click "Launch log".
30. Click the orange "View all instances" for the list of instances at URL such as:
    
    https://us-west-2.console.aws.amazon.com/ec2/home?region=us-west-2#Instances:

31. Click the blue Instance ID for the "mac2.metal" instance you just created. Alternately, select the checkbox for your instance andcClick "Actions" to select View Details.
32. Status check will say "Initializing" initially.

    <a name="PrivateIPAddress"></a>

33. REMEMBER: Connecting to the macOS instance is based on the AWS <strong>Public IPv4 DNS name</strong>, such as:
...
34. Press command+R to refresh until "Status check: Initializing" changes to green "Running".
...
7. If you are idle too long and it times out, ssh again if you see:
...
1.  Set a password for ec2-user:
    ...
    <pre><strong>sudo passwd ec2-user
    </strong></pre>
...
2.  Create a password of sufficient complexity. 8 characters is all that Apple allows. TODO: Store that password in a secrets Vault. Use code to retrieve it so that the password doesn't end up in the command history. So we want the equivalent of:
    ...
    --region eu-central-1 | jq .SecretString -r)
    </pre>

    WARNING: Please make up another password than the above.

3.  Issue a CLI command (via SSH) to enable Sharing after defining VNC_PASSWORD:
...
Done.
    ...
    <pre><strong>echo "$VNC_PASSWORD" | perl -we ‘BEGIN { @k = unpack “C*”, pack “H*”, “1734516E8BA8C5E2FF1C39567390ADCA”}; $_ = <>; chomp; s/^(.{8}).*/$1/; @p = unpack “C*”, $_; foreach (@k) { printf “%02X”, $_ ^ (shift @p || 0) }; print “\n”’ | sudo tee /Library/Preferences/com.apple.VNCSettings.txt
    </strong></pre>
...
1.  On the client computer that controls the VNC server computer remotely, install the <a target="_blank" href="https://www.realvnc.com/en/connect/download/viewer/macos/">Real VNC <strong>viewer</strong></a>:

    <pre><strong>brew install --cask vnc-viewer
    open "$HOME/Applications/VNC Viewer.app"
    </strong></pre>

2.  Click "Open" to the pop-up download alert.
3.  Check to accept its terms, then click OK.
4.  Provide your email and Password "that you used to create your RealVNC account".
...
6.  To disconnect and reconnect to enable the GUI, define :
...
Extend policy-based access control to SSH and sudo:
