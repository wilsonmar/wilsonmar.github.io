...
5.  REMEMBER: AWS does not let users manually release DURING the first day. So you may get this error:
    ...
    The time "T05:48:03" is a 24-hour <strong>UTC</strong> (previously Greenwich Mean Time near London) but with never a Summertime/Dayight Savings change.
...
14. If you have created a Security Group for your IP address, select "Select existing security group" and scroll to pick it.
...
15. Click the arrow in "> Configure storage".
16. Notice that, by default, the instance get 100 GiB of "gp2" (General Purpose SSD) for its Root volume used for boot-up.
...
17. Click "Advanced" at the right to configure EBS Volumes.
18. increase the size of the Root Volume from a default of 60 to 300 GiB (or whatever your capacity analysis runs reveal). 
19. Select Volume Type:
...
20. Add tags per your administrator's guidelines and examples.
...
21. Click the arrow in front of "Advanced Details".
22. For "Shutdown behavior", click "Stop" to select "Terminate" if you are experimenting and want a fresh start every time.

23. Leave these settings to their default unless you'd like to overrride.
...
24. Scroll to the bottom among "Advanced details".
25. If you have commands to be executed upon on the instance's Terminal after boot-up:
...
26. Click the orange "Launch instance" for a message such as:
    
    Success! Successfully initiated launch of instance (i-1234567890abcdef)

27. Optionally, click "Launch log".
28. Click the orange "View all instances"
    
    https://us-west-2.console.aws.amazon.com/ec2/home?region=us-west-2#Instances:

29. Click the blue Instance ID for the "mac2.metal" instance you just created. Alternately, select the checkbox for your instance andcClick "Actions" to select View Details.
30. Status check will say "Initializing" initially.
31. Note the Public IPv4 address, such as <tt>44.234.9.188</tt> which will be different when another instance is created.

32. Press command+R to refresh until "Status check: Initializing" changes to green "Running".

    <a name="ConnectUsingSSH"></a>

    ### Connect using SSH

33. After Instance State is "Running"
34. Notice that "User name" is <tt>ec2-user</tt> ???
35. Click the "Connect" box.
...
36. Click "SSH client" for instructions to appear.

    PROTIP: Instructions below are enhanced version of what AWS presents.

37. Open an SSH client, such as your macOS Terminal.
38. Navigate to a working folder, which you'll be using to upload and download files for your EC2 instances.
39. Define the pem file's location in a variable so you don't have to retype it:
...
41. Set permissions needed:

    <pre><strong>chmod 400 "$PEM_FILE"
    </strong></pre>

42. Click the icon next to the Public DNS, such as:

    <tt>ec2-44-234-9-188.us-west-2.compute.amazonaws.com</tt>

43. Construct the command to save the DNS in a variable:
...
45. Highlight this command and paste in your Terminal to connect SSH:

    <pre><strong>ssh -i "$PEM_FILE" "ec2-user@$PUBLIC_DNS"
    </strong></pre>

46. It takes a few seconds to connect. But if you see:

    <tt>ssh: connect to host ec2-44-234-9-188.us-west-2.compute.amazonaws.com port 22: Operation timed out</tt>

    PROTIP: During development work, terminate instance (and associate Volumes and Dedicated Host) to minimize charges.
...
5.  REMEMBER: Also <a href="#ReleaseHost">release the Dedicated Host</a> to stop charges for accumulting for the current hour.
...
PROTIP: The toil of instance termination and Host release is a good reason to automate the entire process of instance creation.
