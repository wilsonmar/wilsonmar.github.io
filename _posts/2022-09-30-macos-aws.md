...
4.  Construct a Name with your own name, Availability Zone, mac1/2, etc.
...
31. REMEMBER: The Private IPv4 address, such as <tt>172.31.55.215</tt> will be different when another instance is created.

    REMEMBER: Connecting to the macOS instance is based on the AWS DNS name, such as:
...
34. REMEMBER: The "User name" is <tt>ec2-user</tt> which is used by the SSH utility used to log into the macOS instance.
...
38. Proceed to use a Terminal to <a href="#AWSMacConnect">my instructions on how to connect to macOS using SSH</a> are enhanced version of what AWS presents in the step above.
...
6.  Set permissions needed:
   ...
   PROTIP: The approach below makes it easily <stsrong>repeatable</strong>.
...
5.  Use the same command each time:
    ...
    <pre><strong>ssh awsmac
    </strong></pre>

6.  It may take a few seconds to connect. Success looks like this:
    ...
    The "@ip-172-31-55-215" is the <a href="#PrivateIPAddress">Private IPv4 address for the instance</a>.
...
7. Obtain a list of folders and files:

   <pre><strong>ls -al
   </strong></pre>

   WARNING: If you wait too long and it times out, ssh again if you see:

   <pre>client_loop: send disconnect: Broken pipe
   </pre>

   But normally you should see something like:
   ...
   The "Library" folder is where macOS stores its operating system files.
...
10. Create a password of sufficient complexity. 8 characters is all that Apple allows. Store that password in a secrets Vault. Use code to retrieve it. Alternately, for example:
    
    <pre>export VNC_PASSWORD="239$Vacx"
    </pre>

12. Issue a CLI command (via SSH) to enable Sharing after defining VNC_PASSWORD:
...
Done.
    ...
    2. Click the blue "Sharing" icon or type enough of "Sharing" in the Search box at the upper-right to choose it.
...
17. Disconnect and reconnect to enable the GUI:
...
9. Press command+spacebar for Spotlight to search for the screen-sharing app.
