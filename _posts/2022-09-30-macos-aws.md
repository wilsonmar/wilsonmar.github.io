...
60. Optionally, click "Launch log" to see:

    <pre>Initializing requests      Succeeded
    Creating security group         Succeeded
    Creating security group rules   Succeeded
    Launch initiation               Succeeded
    </pre>

1.  Click the orange "View all instances" for the list of instances at URL such as:
    
    https://us-west-2.console.aws.amazon.com/ec2/home?region=us-west-2#Instances:

2.  Click the blue Instance ID for the "mac2.metal" instance you just created. Alternately, select the checkbox for your instance andcClick "Actions" to select View Details.
3.  Status check will say "Initializing" initially.

    <a name="PrivateIPAddress"></a>

4.  REMEMBER: Connecting to the macOS instance is based on the AWS <strong>Public IPv4 DNS name</strong>, such as:
...
5.  Press command+R to refresh until "Status check: Initializing" changes to green "Running".
