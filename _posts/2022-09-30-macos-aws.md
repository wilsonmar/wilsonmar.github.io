...
Now you can operate a full M1 macOS laptop without the hardward, with all your own brews installed, from almost any tablet. Get a mac laptop temporarily to:
...
In 2022 AWS announced availability of on-demand macOS laptops within the AWS EC2 cloud as <a target="_blank" href="https://aws.amazon.com/ec2/instance-types/">EC2 instance types</a>:
...
BTW, this service is built on the <a target="_blank" href="https://aws.amazon.com/ec2/nitro/">AWS Nitro System</a> virtualization infrastructure.
...
PROTIP: If you're using these more that a couple months, you might as well buy your own Mac at $2,500.
...
PROTIP: So it's the convenience and scalability you're paying from cloud instances.

And, within AWS, the ability to communicate with your other resources also running on AWS.
...
Here's what it does:
   ...
   2. Identify an <a name="AvailableMetal">available "m5.metal" instance</a> within an Availability Zone
   3. <a href="#DedicatedHost">Create a Dedicated Host</a> instance in AWS
   4. Create EC2 instance
   5. Install XCode command utilities, brew; use brew to install utilities, run dotfiles to configure macOS; define aliases, etc.
   6. Create EC2 images for each region (if requested)
   <br /><br />
...
## Limited AWS Availability Zones
...
which is rotated through a list of Availability Zones:

<pre><strong>AWS_REGIONS=$( aws ec2 describe-regions --output text --query "Regions[].[RegionName] " | sort -r )
AWS_REGIONS=$( aws ec2 describe-regions --output text --query "Regions[].[RegionName]" | sort -r )
   # | tr "\\n" " "
export AWS_INSTANCE_TYPE="m*.metal"
...
do
   ...
   aws ec2 describe-instance-type-offerings \
      ...
      --output text \
      --filters "Name=instance-type,Values=mac*metal" 
done
</strong></pre>
...
<pre>INSTANCETYPEOFFERINGS   mac1.metal      us-east-1b      availability-zone
INSTANCETYPEOFFERINGS   mac1.metal      us-east-1c      availability-zone
INSTANCETYPEOFFERINGS   mac2.metal      us-east-1d      availability-zone
INSTANCETYPEOFFERINGS   mac2.metal      us-east-1b      availability-zone
INSTANCETYPEOFFERINGS   mac2.metal      us-east-1c      availability-zone
</pre>

Notice that in us-east-1a does not have "mac1.metal" nor "mac2.metal".
