...
PROATIP: Also notice that mac metal is not in all Availability Zones within a Region.
...
You have two ways to get a mac*.metal instance:
   ...
   * <a name="OptionA">Option A: Use CLI</a> (for fast and easy)
   * <a name="OptionB">Option B: Use AWS GUI Manually</a> (not preferred, but good for learning)
   <br /><br />
...
Here's what it does:
   ...
   3. Invoke Terraform commands to automatically:
      4. <a href="#DedicatedHost">Create a Dedicated Host</a> instance in AWS
      5. Create EC2 instance
      6. Install XCode command utilities, brew; use brew to install utilities, run dotfiles to configure macOS; define aliases, etc.
      7. Create EC2 images for each region (if requested)
...
3. View file <tt>mapping.json</tt> for <tt>--block-device-mappings</tt>
...
4. Run the instance using the image-id associated with the region selected:
