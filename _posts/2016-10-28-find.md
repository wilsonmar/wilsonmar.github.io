...
0. Use the GNU find command to look for all files with <tt>.app</tt> file suffix:
   ...
   <pre><strong>find / -type f -name "*.app" 2>/dev/null 1>$HOME/find-all-apps.txt
   </strong></pre>

   <tt>/</tt> specifies all files in the entire drive.
   ...
   <tt><strong>-iname</strong></tt> instead of <tt>-name</tt> would search <strong>insensitive</strong> to case in the specified search string.
