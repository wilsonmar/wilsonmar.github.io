...
An alternative to operating system environment variables is writing program code to read a file such as `properties.xml`. But that means values from them are static across multiple instances, or a mechnism needs to be crafted to change the contents of the file before use.
