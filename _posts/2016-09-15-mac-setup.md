...
8. Press Return/Enter on your keyboard to begin execution. 
   ...
   The design of the script is that a parameter must be specified with the command in order for each fature to be procssed.

   With just <tt>-v</tt> specified, the script returns a menu of options and feature parameters.
...
9. WARNING: The script, by default, downloads a <tt>mac-setup.env</tt> file in your user <tt>$HOME</tt> to hold configuration settings (if it's not already there).

   In future runs, the script makes use of values in variables within that file rather than default values hard-coded in the mac-setup.zsh script file.

9. So after your favorite text editor is installed, edit that <tt>mac-setup.env</tt> file and change the values.

   Once installed, the script does creates a <tt>~/Projects/mac-setup</tt> folder on your machine and downloads the <tt>mac-setup.zsh</tt> script file into it.
...
9. Alternately, edit the configuration files to run <a href="Ansible">Ansible</a>.
...
<a href="#OSDetect">07. Obtain and show information about the operating system in use to define which package manager to use</a>

<a href="#BashVersions">08. Upgrade to the latest version of bash (if applicable).</a>

<a href="#BashTraps">09. Set traps to display information if script is interrupted.</a>
