...
4. Open Safari (the Mac's built-in internet browser) to review the script code at:
   ...
   On the first line, <tt>zsh</tt> is used instead of <tt>bash</tt> to accomodate Apple's new defaults.

   The design of a <strong>single script that does it all</strong> installs everythnig needed: from XCode to Brew to Python.

   Sections in the script are <a href="#ScriptSteps">sequentially numbered in the list below</a>. 
   
   Run-time flags specify actions the script takes.

   Shell coding techniques used in the script are explained in <a target="_blank" href="https://wilsonmar.github.io/shell-scripts">https://wilsonmar.github.io/shell-scripts</a> - aiming to use shell scripting techniques that are the easiest to understand rather than the most "elegant" and esoteric tricks that some others use to prove how superior they are. But if you disagree with something, please let me know. I welcome your suggestions.

5. Open a Terminal by pressing <strong>command+spacebar</strong>, then type enough of <strong>Terminal.app</strong> to select it.

6. Commands make use of the <tt>curl</tt> utility to download. So make you don't get any error about that on the Terminal:
...
7. Use your mouse to triple-click <tt>zsh</tt> in the command below to highlight the line, then press command+C to copy it to your Clipboard:

   <pre><strong>zsh -c "$(curl -fsSL https://raw.githubusercontent.com/wilsonmar/mac-setup/main/mac-setup.zsh)" -v
   </strong></pre>

8. Press <strong>command+Tab</strong> to switch to Terminal. Click on the window. Press <strong>command+V</strong> to paste the command from your Clipboard. Notice that:
