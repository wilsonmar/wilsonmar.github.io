...
My aim is to speed up <strong>onboarding</strong> of new team members and new laptops, so people can get to work without waiting and distractions.

This article explains the automation I created to achieve the fastest way to configure a Mac for DevSecOps-supported software development:

<ul><a target="_blank" href="https://github.com/wilsonmar/mac-setup/">https://github.com/wilsonmar/mac-setup</a></ul>

I reference it now because (in today's hostile world) it's a good habit to know what you are installing on your machine.


{% include whatever.html %}

## TLDR Just Do It Summary

1.  Open a Terminal by pressing <strong>command+spacebar</strong>, then type enough of <strong>Terminal.app</strong> to select it.

    You can be in any folder because my script navigates to a project folder, after creating it if necessary. 

1.  Highlight the line here (use your mouse to triple-click <tt>zsh</tt> in the command below), then press command+C to copy it to your Clipboard:
    ...
    This article (below) presents a guided tour of that shell script file.

1.  Press <strong>command+Tab</strong> to switch between the <strong>Terminal.app</strong> and the browser window.

1,  On the Terminal, press Enter to run the command pasted.

    The command downloads the <tt>mac-setup.zsh</tt> script file from GitHub to your machine and runs it.
...
1. Obtain a <a target="_blank" href="https://www.apple.com/mac/">mac laptop</a> (from Apple, Amazon, or 2nd-hand). Unbox it. Order <a target="_blank" href="https://wilsonmar.github.io/apple-macbook-hardware/">Apple hardware accessories</a>. Arrange it all on your desk for convenient access.
...
5. Commands make use of the <tt>curl</tt> utility to download. So make you don't get any error about that on the Terminal:
   ...
   <pre>curl 8.1.2 (x86_64-apple-darwin22.0) libcurl/8.1.2 (SecureTransport) LibreSSL/3.3.6 zlib/1.2.11 nghttp2/1.51.0
