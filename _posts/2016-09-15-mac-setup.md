...
1.  Optionally, set the prompt so it's consistently on the left after listing the present working directory:

    Highlight the line here (use your mouse to triple-click <tt>zsh</tt> in the command below), then press command+C to copy it to your Clipboard:

    <pre><strong>export prompt_newline="%}"
    export PS1="${prompt_newline}${prompt_newline}  %11F%~${prompt_newline}%% "
    clear
    </strong></pre>

    CAUTION: Do not click on the URL (starting with <tt>httpd</tt>) since the terminal program will then attempt to open a browser to that URL.
   
1.  Command <strong>clear</strong> clears the screen of previous commands.
...
1,  On the Terminal, press Enter Return/Enter on your keyboard to begin execution.
    ...
    <tt>-v</tt> is a parameter that specifies optional verbose log output. <a href="#Parms">Additional parameters are covered later in this article below</a>.
