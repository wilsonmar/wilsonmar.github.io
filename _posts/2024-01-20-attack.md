...
1. Create a SBOM (Software Bill of Materials) of each open-source library package in the chain of references. Flag suspicious version changes, then scan the code of each package for vulnerabilities such as exfiltrating data externally.

1. Verify that each device is running the latest version of its OS and all patches.

1. Use a <a target="_blank" href="https://www.cisecurity.org/cis-benchmarks/">CIS Benchmark</a> to configure each device to a secure state.
