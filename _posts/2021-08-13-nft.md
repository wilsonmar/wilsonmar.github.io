...
The concepts behind Bitcoin was described a year earlier in a whitepaper written anonymously under the name "Satoshi Nakamoto".

Since then, several platforms have appeared to make use of the "blockchain" architecture:
a ledger holds a sequential history (chain) of transactions. A cryptographic hash is made of each transaction and the transaction before it. Any change would be detected.

Git used to enable modern software source version management.

Two approaches have been created.
...
"Smart contracts" is a mechanism that allows common contractual clauses to be specified, verified (enforced) in the absence of trust between contracting parties and in the absence of a third party.
