...
3. Each programming language would have different contents for its <tt>.gitignore</tt> file. Use <tt>cat</tt> or a text editor such as code (for VSCode) to view the file:
 
   <pre><strong>cat .gitignore</strong></pre>
   
   In the <a target="_blank" href="https://github.com/hashicorp/hello-vault-spring/blob/main/.gitignore">.gitignore file for Spring/Java</a>, there would be a spec for contents of folder <tt>target/</tt> generated during each run is not uploaded to GitHub. Several target folders are generated. Several files generated by STS, IDEA IDE, NetBeans, and VSCode are also not uploaded.

4. If you have not already done so, create a <tt>global .gitignore</tt> configuration file on your machine $HOME folder:
...
1. Highlight and copy these lines and paste into your global .gitignore file:
...
1. Consider additional gitignore patterns at: 

   <a target="_blank" href="https://github.com/github/gitig">https://github.com/github/gitig</a>
