...
excerpt: "Automate creation of Windows server as VMware Fusion images used to make Vagrant boxes for the HashiCorp Atlas repository"
...
This tutorial is rather dated because many are migrating from Vagrant images to Docker.

Vagrant has been around longer than Docker. So some consider it more "mature", stable, "simpler to understand" than Docker.

Windows 8.1 and Windows 10 Business editions came with Hyper-V installed.
...
Individual Vagrant box images are Gigabytes (many times) larger than Docker "containers", and thus take more RAM as well. This is because Vagrant box images contains the entire operating system whereas Docker separates out shared components. See my ["Docker setup tutorial"](/docker-setup/).
...
1. Click "Download Now" for the installer (not the Pro edition) from:
...
1. In Finder folder "/Applications", if it's there, right-click on an existing VMWare Fusion and Move to Trash.
