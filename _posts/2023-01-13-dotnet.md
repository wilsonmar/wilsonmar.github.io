...
excerpt: "Write C# Azure Functions and cross-platform .NET Core apps within the Microsoft ecosystem"
tags: [microsoft]
image:
# ms-dot-net-auto-patche-1900x500.png
  ...
  credit: Graham Wardale, Nissan UK
  creditlink: https://www.trendhunter.com/trends/longest-car
comments: true
---
<i>{{ page.excerpt }}</i>
{% include l18n.html %}
...
Job descriptions for a "Microsoft full stack developer" mention dozens of technologies. Here is the order to make sense of <a target="_blank" href="https://www.altexsoft.com/blog/engineering/the-good-and-the-bad-of-net-framework-programming/">them</a>:

1. <strong>.NET</strong> is the ecosystem Microsoft built (beginning in 2002) around an architecture similar to Java, where
2. <strong>CIL</strong> (<a target="_blank" href="https://www.wikiwand.com/en/Common_Intermediate_Language">Microsoft Intermediate Language</a>, aka "bytecode") is created from compiling source code written in various human-readable programming languages (C#, Visual Basic). MSIL is read by Microsoft's proprietary
3. <strong>CLR (Common Language Runtime)</strong>, which is equivalent to the JVM (Java Virtual Machine) that turn bytecode into native code executed by specific computer processors (x86, ARM, etc.).

   In theory, this is instead of distributing different executable files for different platforms and CPU types. 
   However, ...
...
6. <strong>.NET 8</strong>, also known as "<strong>Modern .NET</strong>", unifies legacy platforms and standards"https://github.com/markjprice/cs12dotnet8/blob/main/docs/ch01-dotnet-history.md combines and thus <a target="_blank" href="https://devblogs.microsoft.com/dotnet/the-future-of-net-standard/">replaces</a>:
   ...
   * <a target="_blank" href="https://docs.microsoft.com/en-us/dotnet/standard/net-standard">.NET Standard</a> ("netstandard") library  
   * <a target="_blank" href="https://learn.microsoft.com/en-us/aspnet/core/?view=aspnetcore-7.0">ASP.NET Core</a> is for web development using the MVC (Model-View-Controller) programming pattern (equivalent to Node.Js)
...
24. More granular than RBAC is ASP.NET Core Identity Roles, which  supports CBAC (Claims-Based Access Control) enforced through Authorization Policy stored in the AspNetUserClaims table.<a target="_blank" href="https://app.pluralsight.com/course-player?clipId=124c89b6-9d3d-4f87-9859-dc7747bd0e21">*</a>

25. <a target="_blank" href="https://learn.microsoft.com/en-us/dotnet/architecture/dapr-for-net-developers/">Dapr (Distributed Application Runtime)</a> apps are cloud-native event-driven, portable runtimes that can run within Kubernetes on most clouds

26. <strong>MS-SQL</strong> is the database from Microsoft that runs on many platforms.
27. <strong>Azure Cosmos DB</strong> runs across regions in the Azure cloud
28. <strong>LINQ</strong> is Microsoft's proprietary extension to the SQL language for querying and manipulating data within relational (SQL) databases.
29. <strong>EF (Enterprise Framework) Core</strong> enables  object-to-data store mapping when programming reading and writing to relational data stores (such as SQLite and SQL Server).
...
To see a list of templates:
   ...
   <pre><strong>dotnet new list</strong></pre>

   <pre>Read more about .NET CLI Tools telemetry: https://aka.ms/dotnet-cli-telemetry
&nbsp;
----------------
...
1. Enter vue in the Search for packages… textbox 

   note that about 210 packages are returned.

   <pre>dotnet new --install "Vue.Simple.Template"
...
7. Install dotnet-sdk
   ...
   <pre>==> Downloading https://download.visualstudio.microsoft.com/download/pr/dfbffbab
######################################################################## 100.0%
...
1. When no errors are returned, run the service:
   
   <pre><strong>docker-compose up
   </strong></pre>

2. Pop up browser window for each component:
...
https://gayanf.medium.com/deploy-eshoponcontainers-to-aks-357ac48cda60
Deploy eshopOnContainers to AKS

https://faun.pub/eshoponcontainers-experiment-with-image-scanning-21b73ac5c4b7
eShopOnContainers experiment with image scanning using <a target="_blank" href="https://github.com/aquasecurity/trivy-action#customizing">Trivey</a>.
by editing the .github/workflows/basket-api.yml file from the eShopOnContainers repository and add a step between the build and push step.
...
YouTube: <a target="_blank" href="https://www.youtube.com/watch?v=J0FhV3dM80o">Every single feature of C# in 10 minutes</a>
...
* use ASP.NET Core to build web apps on browsers coding Razor, Blazor for native WebAssembly on browsers, with constant connection with server via SignalR 
   * <a target="_blank" href="https://www.youtube.com/watch?v=oyH3G_ntpKk">VIDEO: Build browser extensions with Blazor</a>
   * <a target="_blank" href="https://www.youtube.com/watch?v=QlRLz3xJmxs">Dark mode</a>
...
* C# is a strongly-typed language, but <tt>var</tt> has the compiler infer the type implicitly.
* Compound assignment operators Increment: <tt>value++; value += 5; </tt>  -- *=

   <pre>Console.WriteLine($"Second: {value++}");  # adds line break
...
* Functions:
   * <a target="_blank" href="https://www.youtube.com/watch?v=4uwRXA3oUSQ">VIDEO: Rewriting Functions</a>
   * <a target="_blank" href="https://www.youtube.com/watch?v=eXaf9Egu7q0">Clean functions</a>
* Collections:

   <pre>using System Collections;  # namespace at top of file
   var myList = new List&LT;string>();
   var myDictionary = new Dictionary&LT;int, int>();
   var myQueue = new Queue&LT;string>();
...
* <a target="_blank" href="https://www.youtube.com/watch?v=G3XqSaF1CrY">Arrays</a>:

   <pre>var myArray = new string[5];
   </pre>

* switch statements can be written without case statements

   <pre>string MyFunctionA(int value) ==>
   value switch {
      1 => "one",
      2 => "two",
      > 10 => "large number",
...
* Generics with restrictions
   ...
   <pre>class MyGenericClass&LT;T>() where T : IThing {
      return typeof(T).GetType().Name;
      }
   </pre>

* <a target="_blank" href="https://www.youtube.com/watch?v=E2StkoEjbkE">Record types</a>

* Patterns:
   * <a target="_blank" href="https://www.youtube.com/watch?v=oaM3h7OY5lM">Dependency Injection</a>
   * <a target="_blank" href="https://www.youtube.com/watch?v=e5pnfD0rudY">Circuit Breaker</a>
...
<a target="_blank" href="https://www.freecodecamp.org/learn/foundational-c-sharp-with-microsoft/">https://www.freecodecamp.org/learn/foundational-c-sharp-with-microsoft</a>
<a target="_blank" href="https://www.youtube.com/watch?v=6GQAE7iLOhY">Foundational C# with Microsoft text-based course on Microsoft's LEARN platform and certification on FreeCodeCamp.dev by <a target="_blank" href="https://www.youtube.com/watch?v=MNw6_IlvXb8">Gavin Lon</a>
<a target="_blank" href="https://devblogs.microsoft.com/dotnet/announcing-foundational-csharp-certification/">Announced Aug 28, 2023</a>.
The challenge concluded in Oct 28, 2023.
...
Classes on Secure C# Coding:
   * <a target="_blank" href="https://app.pluralsight.com/library/courses/c-sharp-10-owasp-secure-coding/table-of-contents" title="20 Sep 2022">Pluralsight: Secure Coding with OWASP in C# 11 & .NET 7</a> (Visual Studio 2022) by <a target="_blank" href="https://www.linkedin.com/in/atushinsky/">Alexander Tushinsky</a><a target="_blank" href="https://www.atushinsky.com/">.com</a>
...
Pluralsign: <a target="_blank" href="https://app.pluralsight.com/library/courses/building-microservices/table-of-contents">Building Microservices</a> 29 Jun 2021
by Mark Heath 
...
<a target="_blank" href="https://www.youtube.com/watch?v=Y8JKVjY-7T0" title="Nov 13, 2016 by Eric Johnson">VIDEO</a>:
"Continuous Integration: Live Static Analysis using Visual Studio & the Roslyn API"</a>

The .NET Compiler Platform ("Roslyn API") was first described in 2014 by Alex Turner in 
...
<a target="_blank" href="https://app.pluralsight.com/course-player?clipId=a2b0387f-162e-4436-9977-1c623b4c59a8">PROTIP</a>: 
Use Program.cs NWebSec middlware code library for secure HTTP Header handling. See https://docs.nwebsec.com/en/latest


<a target="_blank" href="https://learning.oreilly.com/videos/writing-secure-code/9781803246123/" title="Packt, August 2021
5h 58m">BOOK: Writing Secure Code in ASP.NET</a>
by <a target="_blank" href="https://www.linkedin.com/school/cyberacademy-co/">Cyber Academy</a>

<a target="_blank" href="https://www.linkedin.com/learning/asp-dot-net-security-2/">COURSE: ASP.NET: Security</a> on LinkedIn Learning by <a target="_blank" href="https://www.linkedin.com/in/christianwenz/">Christian Wenz</a> shows OWASP10 security vulnerabilities, common attacks, and remediation examples in his known-bad e-commerce app written in ASP.NET." Packt Publishing December 2023, 798 pages
references https://github.com/markjprice/apps-services-net8 and https://static.packt-cdn.com/downloads/9781837637133_ColorImages.pdf
...
<a target="_blank" href="https://learning.oreilly.com/library/view/apps-and-services/9781837637133/">BOOK: Apps and Services with .NET 8 - Second Edition
By Mark J. Price 
