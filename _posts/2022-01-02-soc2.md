...
There are both full and lite editions of CAIQ. The full 310 questions in v3.1 and 261 questions in v4.0 map validated controls to 16 control categories, in this by code alphabetical order:

1. <a href="#AAC-">AAC</a>  (Audit Assurance & Compliance)
...
\1. <a name="AAC-"><tt>AAC-</tt></a>  (Audit Assurance & Compliance)
...
\2. <a name="AIS-"><tt>AIS-</tt></a> (Application & Interface Security)

   Because Consul issues <strong>Health Checks</strong> of services, Consul is able to route  <strong>traffic only to healthy app services</strong>.
   <br /><br />

\3. <a name="BCR-"><tt>BCR-</tt></a>  (Business Continuing Management & Operational Resilience)

   Enterprise Consul is configured to use Consul <strong>Redundancy Zones</strong>, which ???
   <br /><br />

\4. <a name="CCC-"><tt>CCC-</tt></a>  (Change Control & Configuration Management)

   We configure Enterprise Consul to perform <strong>upgrades</strong> automatically. On AWS, Consul works with ASG (Auto Scaling Groups).
   <br /><br />

\5. <a name="CEK-"><tt>CEK-</tt></a>  (Cryptography, Encryption, and Key Management)

   We configure Enterprise Consul to use a Consul Network Mesh to provide <strong>mTLS (Mutual TLS)</strong> communicatons between app services and also with services (through a Gateway) on a wide variety of services:
   <br /><br />

\6. <a name="DCS-"><tt>DCS-</tt></a>  (Datacenter Security)

\7. <a name="DSI-"><tt>DSI-</tt></a>  (Data Security & Information Lifecycle Management)

   We use the cloud region appropriate to the data laws of each country. Sensitive data of German citizens are stored in servers in a region in Germany.
   <br /><br />

\8. <a name="GRC-"><tt>GRC-</tt></a>  (Governance, Risk Management, and Compliance)

   We use Consul to control routing of traffic among apps based on Intentions and ACL (Access Control List) in the Consul KV (Key Value) store. Intentions determine the identities which an app may communicate with. ACLs determine whether specific permissions (such a Read, Write) are allowed or denied.
   <br /><br />

\9. <a name="HRS-"><tt>HRS-</tt></a>  (Human Resources)

\10. <a name="IAM-"><tt>IAM-</tt></a>  (Identity & Access Management)

   Instead of blindly routing to IP addresses, our apps use Consul which uses cryptographic certificates (mTLS) to authenticate with services.
   <br /><br />

\11. <a name="IPY-"><tt>IPY-</tt></a>  (Interoperability & Portability)

   Because Consul is configured with uses Health Checks, Consul is able to only route traffic to healthy app services.
   <br /><br />

\12. <a name="IVS-"><tt>IVS-</tt></a>  (Infrastructure & Virtualization Security)

   We use Consul to automatically discover IP addresses and other metadata as soon as a service comes online.
   <br /><br />

\13. <a name="LOG-"><tt>LOG-</tt></a>  (Logging and Monitoring)

   Consul emits a log for each transfer.

   Logs emitted from Consul are routed to a SIEM collector.
   <br /><br />

\14. <a name="SEF-"><tt>SEF-</tt></a>  (Security Incident Management, E-Discovery, and Cloud Forensics)

\15. <a name="STA-"><tt>STA-</tt></a>  (Supply Chain Management, Transparancy, and Accountability)

   We follow the same standards that we ask our vendors follow.
...
\16. <a name="TVM-"><tt>TVM-</tt></a>  (Threat and Vulnerability Management) 

   These reference other standards, such as data-labeling standards ISO 15489, Oasis XML Catalog Specification, CSA.
   <br /><br />

\17. <a name="UEM-"><tt>UEM-</tt></a>  (Universal Endpoint Management) 
