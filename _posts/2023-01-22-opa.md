...
This article is a deep-dive tour with commentary at each step along the way.
...
3.  Click the blue "Evaluate" button to see OUTPUT generated.
    ...
    Varible key "message" is an attribute of "input".
...
6.  Click "Kubernetes", then "Label Existence". 
    ...
    PROTIP: Such is often the first policy to be added to control cloud usage in resources created from Terraform.

7.  PROTIP: For each error message, create a web page to show how to fix the error.

    ### Analyze JSON INPUT

1.  <a target="_blank" href="https://play.openpolicyagent.org/p/0FjgydMuVo">This example</a> ensures that,  when PUT methods are used, the user and owner are the same when value "pets" is <strong>somewhere</strong> within the path:
...
1.  Achieve <tt>"allow": true</tt> by changing one email to be the same as the other email:
