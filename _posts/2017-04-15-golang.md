...
1. Alternately, "beta" as of April, 2017 is a paid, commercially supported, quality-assured Go distribution
   ...
   PROTIP: 
   Instead of downloading installer from website (and managing versions yourself) from:
...
1. On a Mac, install <a target="_blank" href="https://brew.sh/">Homebrew</a> by triple-clicking this command for copy to Clipboard (using command+C):
...
1. Confirm the go executable works:
   ...
   <pre>go version go1.14.4 darwin/amd64
   </pre>
...
1. To run a specific version of Go:
...
0. To create a new bug (issue) in the language, use a built-in command to open the repository holding source code behind the Go language, in a your default browser application:
   ...
   Notice results from go version and go env are automatically pasted there.
...
CGO_LDFLAGS="-g -O2"
   ...
   `GOROOT` is in Cellar because it was installed by Homebrew. Notice it's for a specific version of Go. So alternately: <tt>/usr/local/opt/go/libexec/bin</tt>
...
Generally a Go <strong>workspace</strong> is created for each project. 

Each Go workspace directory must have three sub-directories: 

   * src folder contains Go source code UTF-8 files with ".go" ending. Under this folder is the `github.com` folder and the account name and repo name cloned by a `go get` command.

   * pkg folder contains Go package objects named "package-name.a" (for archived) named in imports within the source code. Such files are under a folder for its machine architecture (such as "darwin_arch64").  Each package object contains the compiled package binary code, along with debugging symbols and source information. These files are created by the Go pack tool (compiler) while building/installing a package. Go uses packages here instead of the default packages folder $GOROOT/src or $GOPATH/src directories. 

   * bin folder contains binary files created by the `go install` command which trigger a `go build` command. The `$GOBIN` environment variable points to this folder as well as `$GOPATH/bin`.
   The default installation directory on macOS is  `/usr/local/go`.

1. Open a text editor to edit (~/.bash_profile on Macs) to (replacing wilsonmar with your 
   GitHub account name):
...
Use the equivalent of "git clone" to download slides and code to the <a target="_blank" href="https://learning.oreilly.com/videos/testing-in-go/">"Testing in Go" video class</a> by Brian Ketelsen:
   ...
   $GOPATH by default points to the $HOME/go directory in UNIX and 
...
//        go run sample.go mul -a=25 -b=4
   ...
   Put `.idea` in your project's `.gitignore` file.
   ...
   * Visual Studio Code has a <a target="_blank" href="https://code.visualstudio.com/docs/languages/go">
   ...
   To write better code and make formatting/debugging easier, install the VSCode extension to work with Go <a target="_blank" href="https://github.com/golang/vscode-go">https://github.com/golang/vscode-go</a> (formerly https://github.com/Microsoft/vscode-go)

1. In VSCode, click the extensions icon at the bottom of the left menu.
...
1. Click "Shell Command Install 'code' command in PATH.
   ...
   GOROOT="/usr/local/go" is where downloaded packages install Go.

1. Install <a target="_blank" href="https://marketplace.visualstudio.com/items?itemName=JFrog.jfrog-vscode-extension">JFrog Visual Studio Code extensions</a> which reveals in <a target="_blank" href="https://jfrog.com/blog/gocenter-reveals-go-module-vulnerabilities-with-xray/">JFrog's GoCenter online</a> security-related metadata about each reference in your go.mod (modules) files.The XRay Software Composition Analysis Tool (SCAT) is based on <a target="_blank" href="https://vulndb.cyberriskanalytics.com/#statistics">over 200,000 vulnerabilities</a> identified and codified by Risk Based Security [RBS] in its VulnDB vulnerability database. It displays indirect (transitive) dependencies, in a hierarchical tree view.
