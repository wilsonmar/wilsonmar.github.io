...
Go is generally seen as not an appropriate fit for iOS, JS.


## Install Compiler

1. See the web home page for the language:
...
1. What is the latest versions of Go? Look at where it's open-sourced:
...
1. Alternately, "beta" as of April, 2017 is a paid, commercially supported, quality-assured Go distribution
   ...
   Install using third-party software installers so new versions can be managed automatically from the command-line.
...
🍺  /usr/local/Cellar/go/1.13.4: 9,271 files, 414MB</pre>
   ...
   PROTIP: Remember to upgrade after every macOS version upgrade (to Big Sur, etc.).
...
1. To upgrade to the latest on macOS using Homebrew:

   <tt><strong>brew upgrade go
   </strong></tt>

2. If there was a previous version installed:
...
0. Get detailed advice about writing Go while verifying whether you can do Go commands:
...
0. If you see a colon at the lower-left corner:
   ...
   * press `q` to quit.
   ...
   ### File an Issue for Go on GitHub

0. To create a new bug (issue) in the language, use a built-in command to open the repository holding source code behind the Go language, in a your default browser application:
...
0. Switch back to the Terminal (press command+tab).
...
## Source Code Editors
   ...
   * when using IntelliJ, put `.idea` in your project's `.gitignore` file.
   ...
   <pre><strong>brew cask install goland</strong></pre>
