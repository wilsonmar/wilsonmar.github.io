...
CGO_LDFLAGS="-g -O2"
   ...
   <tt>echo $GOROOT</tt> to see where downloaded packages are deposited. That's:
   ...
   `GOROOT` is in folder "usr/local/Cellar" because it was installed by Homebrew. Notice it's for a specific version of Go. So alternately: <tt>/usr/local/opt/go/libexec/bin</tt>
