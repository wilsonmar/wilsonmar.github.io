...
## Test-Driven Development
   ...
   PROTIP: When you create a new go program file, also create a companion <tt>_test</tt> file<a target="_blank" href="* https://github.com/quii/learn-go-with-tests">*</a>:
...
&nbsp;
   ...
   Log, Error, and Fatal are different levels of severity. Error is Log followed by `t.Fail`.

   `t.Error` and "t.Errorf" fails out (and stops) on error.

   The "f" as in t.Logf and t.Errorf and t.Fatalf specifies use of `%v` for strings.

   `t.Error*` continues on error.
  
   PROTIP: Using assert statements with variables enable you to copy and paste and code less.
   This also makes it more readable.

   `t.FailNow`, `t.Failed`, `t.Name`
