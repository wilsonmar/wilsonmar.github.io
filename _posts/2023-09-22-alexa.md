...
PROTIP: Instead of the Alexa-branded Smart Home devices from Amazon, I recommend 3rd-parties who created devices that work with both Alexa and Google Home:
   ...
   * Phillips Hue bulbs and other devices
...
1.  The Alexa-enabled device passes the recording file to the Amazon Alexa Service. The Alexa service applies to each recording its Speech Recognition (ASR) and Natural Language Understanding (NLU) software to come up with the "<strong>intent</strong>" which back-end servers understand. Part of that is identifying the specific "<strong>skill</strong>" (app) can fulfill specific "<strong>task</strong>". 
    
1.  Intents are handled by a Skill app written by programmers. The set of words and phrases people use to communicate with a skill is called an "<strong>interaction model</strong>". 
