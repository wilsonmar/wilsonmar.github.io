...
## Developer Buzzwords
    ...
    Then the service checks what <strong>skill</strong> (app) can handle that task. 
    
    Internally, intents are defined by a <strong>skill</strong> written by programmers.

    The service sends the request to the skill's hosting service. The hosting service executes the skill code and returns a response, including text for Alexa to speak. 
...
1.  Choose a memorable, descriptive skill name – For example, users would remember "Pizzabot" and know that it's a food-ordering skill. Likewise, "United States Quiz" is a good name for a history trivia skill.
    ...
    * What is the special today in the cafeteria/restaurant?
    <br /><br />
...
1.  Click Next to click "Learn more" before choosing a <strong>template</strong> as a convenient start toward a new skill. These templates automatically set up your basic skill framework and components. 
    ...
    * "Scheduling Skill" Build a skill to allow users to schedule appointments on your calendar, receive email confirmations and reminders. Includes: voice permissions, reminders, API calls, session persistence. By Dabble Lab

    * "Survey Skill" Build a stand-up or survey skill that <strong>uses passcodes</strong> to allow only authorized users to provide updates and respond to questions. Includes: using API calls, session persistence By Dabble Lab.
...
Choose a pre-built or custom interaction model – The interaction model consists of the words and phrases people use to communicate with your skill. 
...
<a target="_blank" href="https://www.youtube.com/watch?v=RzS69XCe9cw">Build Statue of Unity Facts skill</a> from Rahul Patel 

It's based on the Hello World template.
