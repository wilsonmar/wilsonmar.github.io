...
1.  <a target="_blank" href="https://www.youtube.com/watch?v=NHswkfcpz_M&t=4m9s">VIDEO</a>: Turn off "Drop In". Repeat for each device. In the Alexa app,  select Device Settings. Select your device. Select Communications. Select Drop In. Select On. Select Household Only. Select Save.
    ...
    DEFINITIONS: An "utterance" is a spoken phrase that invokes an "intent". 
    
    Each utterance maps to only one intent. 

    The Alexa-enabled device passes user intents (requests) to the Alexa service. 
    Alexa determines what the user wants or intends to do. 
    Then the service checks to see if the skill can handle that task. 
    If so, it sends the request to the skill's hosting service. The hosting service executes the skill code and returns a response, including text for Alexa to speak. 
...
1.  Tell Alexa "set whisper mode", and Alexa will keep responding in a whisper back to you if you whisper to it.
    ...
    PROTIP: Ask common actions with just or two words:
    ...
    * The <a target="_blank" href="https://www.cnet.com/products/anova-precision-cooker-bluetooth-plus-wi-fi/review/">Anova Precision Cooker</a>'s Alexa skill can look up cooking guides and begin cooking using your voice. 
       ...
       * Alexa, ask Anova to increase temperature by 2 degrees Fahrenheit
       <br /><br />
...
1.  Ask Grubhub / Instacart to reorder my last order
    ...
    REMEMBER: Skills that have several steps you need to repeat the skill name with every utterance. This will be fixed soon by Amazon I hope.
    ...
    Multi-step enables interactive game play:
