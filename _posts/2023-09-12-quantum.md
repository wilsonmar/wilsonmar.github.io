...
Identify risk by periodically running a password cracking tool against your organization's passwords (if they are granted permissions to like Active Directory, etc..

One candidate tool is https://github.com/smaranjitghose/quantum_burglary
which is the quantum equivalent of the <a target="_blank" href="https://hashcat.net/hashcat/">Hashcat</a> password-cracking tool using classic software and hardware (Windows, Linux, and macOS). See https://hashcat.net/forum/thread-9818-page-2.html
...
Having samples would also identify the level of complexity (number of digits, special characters, etc.) that would be required to make a password meet the corporate policy.

To minimize costs, keep track of what passwords are cracked and what passwords are not cracked, based on a hash of each password. 
