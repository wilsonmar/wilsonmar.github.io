...
<strong>"Circuit Width"</strong> refer to techiques for optimizing circuits. Many quantum circuits can be made more efficient if they have access to <strong>ancilla bits</strong> used as a workspace. A single ancilla in an unknown ("dirty") state can cut the cost of incrementing a register in half.
