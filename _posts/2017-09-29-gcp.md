...
<table border="1" cellpadding="4" cellspacing="0">
   ...
   <tr valign="top"><td> Users manage: </td><td> One container per VM </td><td> Like on-prem. K8s</td><td> yaml </td><td colspan="2">No-ops</td><td>Managed</td><td> No-ops </td></tr>
   ...
   <tr valign="top"><td> Language support: </td><td> Any </td><td> Any </td><td>Any</td><td>Java, Python, Go, PHP</td><td>Any</td><td> - </td><td> - </td></tr>
   <tr valign="top"><td> Primary use case: </td><td> General computing </td><td> Container-based</td><td>Containers </td><td>Web & Mobile apps</td><td>Container-based</td></tr>
</table>
...
* <a href="#GCE">Compute Engine</a> (GCE) is a managed environment for deploying virtual machines (VMs), providing full control of VMs for Linux and Windows Server. The API controls addresses, autoscalars, backend, disks, firewalls, global Forwarding, health, images, instances, projects, region, snapshots, ssl, subnetworks, targets, vpn, zone, etc.
...
* <a href="#GAE">App Engine</a> (GAE) is a managed serverless platform for <strong>deploying application code</strong> for Google to run at scale

* Cloud Run enables stateless containers, based on KNative. Being in a container means any language can be used. Each container listens for requests or events.

* <a href="#GCF">Google Cloud Functions</a> (previously called Firebase, like Amazon Lambda) is a managed serverless platform for deploying event-driven functions. It runs single-purpose microservices written in JavaScript executed in Node.js when triggered by events.
