...
3. Visit <a target="_blank" href="https://helm.sh/docs/">https://helm.sh/docs</a>
   ...
   Until Helm3 released November 2019, a <strong>Tiller server</strong> runs inside your Kubernetes cluster to manage (install, upgrade, query, and remove) Kubernetes resources via calls to the Kubernetes API server. <a target="_blank" href="https://helm.sh/docs/faq/#removal-of-tiller">[1]</a> Thus, <tt>helm init</tt> was removed as well.

   See https://github.com/helm/helm-2to3 for the strangler patter (co-existing in the same cluster) or <em>in situ</em> (with migration).

   <pre><strong>helm3 plugin list</strong></pre>

   Helm 2 used <strong>ConfigMaps</strong> (by default) to store release information. 

   In Helm 3, <strong>Secrets</strong> are now used as the default storage driver.
...
Data:   $XDG_CONFIG_HOME - ${HOME}/<strong>.local</strong>/helm/
   ...
   Helm 3 puts CRD's (Custom Resource Definitions) in the "crds" directory and can be skipped using <tt>\-\-skip-crds</tt> on install. https://github.com/bitnami-labs/helm-crd is not under active development.
...
1. Examine the files created using a Tree command:
   ...
   PROTIP: <tt>apiVersion</tt> is v2 starting with Helm3. Sorry for the confusion.
