...
Switch among them using command+tab on Mac or Alt+tab on Windows.


## Contributing to open source

<a target="_blank" href="images/git-flow-2018-08-30b.gif">
<img width="650" src="images/git-flow-2018-08-30b.gif"><br />(Click here for full screen)</a>
...
Here we have a repository to which we have <strong>no right</strong> to alter. 
The repo is addressed by a standard address like others on the internet.
But if you <strong>fork</strong> the repo under your own account, you would have the rights to make changes.
On your <strong>local machine</strong> install a <strong>Git client</strong>. On the Windows machine is installed a <strong>git bash</strong> command line terminal that works like mac and Linux Bash.
The command <strong>cd with a tilde</strong> character changes your present working directory to your account folder. With Bash there is a file named <strong>.bash_profile</strong> which defines environment variables and <strong>aliases</strong>. Configure how Git works by <strong>git config</strong> global commands to set in a <strong>.gitconfig</strong> file your name, email address, and on Windows, how to store line endings.

<a name="HandsOn"></a>

Now let's get hands-on typing each of the commands in the sequence presented in the video.
...
0. Open a personal email account if you haven't already.
...
0. Identify a public GitHub repository and something that you can contribute to, such as suggesting a typo fix. But you have no right to edit that repo. For example:
...
0. Create a GitHub repository, such as you account's github.io website.

   <a name="GitClient"></a>
   
   ### Git client install

   To work with repositories on your local machine, you have to install a Git client. 

   There are several companies that provide clients. I list them and how to install them in my class handout.
...
0. Its installation includes adding in your user home folder a <strong>.gitconfig</strong> file.
   That file stores Git global configurations such as username and userid settings for attribution. 
...
git config --global user.id "wilsonmar@gmail.com"
   ...
   On a Windows machine, mute warnings about conversion of line endings:
...
git config --global core.safecrlf false
   ...
   PROTIP: There are many other Git configuration commands. I have a script that installs them and other apps on a Mac according to a specification file at https://github.com/wilsonmar/mac-setup/master/blog/mac-install-all.sh

   <a name="Terminal"></a>

## Terminal (vs GUI)

   Git commands are typed into a Terminal shell window on a Mac, a Git Bash command window on Windows, or a Run command window on Windows.
...
0. PROTIP: When a repo is cloned locally, Git does not create a folder for the account.

   So create a folder in which to clone (create) repositories. 
