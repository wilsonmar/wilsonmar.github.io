...
* <a target="_blank" href="https://github.com/wilsonmar/wilsonmar.github.io/blob/master/images/git-flow-v05.mp4?raw=true">Click this link to pop-up a video of the diagram in another window of your default internet browser</a>
* <a target="_blank" href="https://grastorftest.com/video.html">Click this link to download a video (11 MB mp4) of the diagram</a> 
so you can click the Pause button or drag the timing bubble on the video's timeline. Enjoy!

<!--
<a target="_blank" href="https://s3.us-east-2.amazonaws.com/mar-grastorf/wilson.mp4">Click here for a video of the diagram to pop-up in another window</a>
<a target="_blank" href="https://wilsonmar.github.io/images/git-flow-2018-08-30b.gif">
<img width="650" src="https://wilsonmar.github.io/images/git-flow-2018-08-30b.gif"><br />(Click here for full screen</a>. 
-->
...
## Use several browsers and screens
   ...
   * <a target="_blank" href="https://www.code2bits.com/how-to-install-chrome-on-macos-using-homebrew">Install the Chrome browser</a> to view <a target="_blank" href="https://wilsonmar.github.io/git-flow">this tutorial page</a> and the <a href="#Video">pop-up video</a> because Chrome suports the latest web technologies:

   <pre><strong>brew cask install google-chrome</strong></pre>

   * <a target="_blank" href="https://www.code2bits.com/how-to-install-chrome-on-macos-using-homebrew">Install the Firefox browser</a> to <strong>view another account's upstream repository</strong> on GitHub/GitLab:
   
   <pre><strong>brew cask install firefox</strong></pre>

   * <a target="_blank" href="https://brave.com/download/">Install the Brave browser</a> to work with <strong>your own repositories</strong> on GitHub/GitLab:
   ...
   PROTIP: Switch among programs using command+tab on Mac or Alt+Esc on Windows:
   
   * Video (poped up by clicking under the <a href="#Flowchart">flowchart</a>)
   * Text editor (if you want to edit the script)
   <br /><br />

   Switch among among tabs of same program using command+` (back tic) on Mac or Alt+tab on Windows.

   PROTIP: Ideally, you would have each of these on a separate physcial screen.


### Two tabs Terminal

1. On a Mac, open a Bash terminal by holding down command key, then press Tab for the Spotlight. Type ter. When "Terminal.app" appears, press Enter to select it.

   On Windows, press the Micosoft icon on the keyboard. Type git. When "git bash" appears, click on it.

2. PROTIP: Open a second Bash terminal session
...
1. Click the command below until the entire line (which wraps around) is highlighted:
...
1. Press Enter to run it.
   ...
   ">>>" describes each section and step of the script:

   1.x <a href="#MacInstall">Mac client Install and Configuration</a>
   
   2.x <a href="#GitConfig">Git Setup and Configuration</a>
   
   3.x <a href="#Cloud">Cloud repository setup</a>

   4.x <a href="#CloneLocal">Clone locally</a>

   5.x <a href="#Checkout">git checkout new branch</a>
   ...
   "$" precede commands issued by the script.
...
6. One can create a Git repository by creating a folder and running a command:

   <pre><strong>mkdir local-repo && cd local-repo
   git init
   </strong></pre>

   This would create a folder always named <strong>.git</strong>
...
BTW setting up two-factor authentication <strong>(2FA)</strong> to provide a number from an authentictor app on your smart phone (such as Google Authenticator) is good security practice. 


   ### Setup SSH

   The right to alter a cloud repo is claimed by a <strong>public key</strong> pasted in the cloud GUI from a <strong>public key file</strong>. The "rsa" in the name refers to the clever mathematicians who figured out how to encrypt and decrypt files without sending passwords. It works by generating two files -- the public and private keys. The files are generated into your account's <strong>~/.ssh</strong> folder because SSH (secure shell) is the same mechanism the Linux team uses to secure its bash terminals. 

   In fact, Git was designed by Linus Torvold and his team who developed the Linux kernel and its secure shell (SSH).

   So the <strong>ssh-keygen</strong> is built in Linux and Mac. Windows users need to download the equivalent <strong>puttygen</strong> program. When there are multiple accounts, a <strong>config</strong> file sorts them out. 

   The Git client uses the private key in the <strong>id_rsa</strong> file to encrypt what it sends. 

   The GitHub or GitLab cloud service decrypts using the public key.

1. List the files in folder .ssh:
   ...
   If click <strong>fork</strong> into your own GitHub account, you would then <strong>have a right</strong> to change the contents of the repository.

   * For example, if your own account is "wilsonmar", your copy of the repo would be:<br />
   <pre>https://github.com/wilsonmar/some-repo</pre>
   <br /><br />

   ### Fork using Hub add-in to Git

   <a target="_blank" href="https://x-team.com/blog/speed-up-your-github-workflow-with-hub/">This blog</a> describes installation and use of a simple wrapper around Git’s CLI so you can do it all from the command line rather than switching to GitHub’s web page.

   <a target="_blank" href="https://www.youtube.com/watch?v=jeefpcqgUvY">VIDEO</a>: People usually fork a repo by manually pressing "Fork" while on the GitHub/GitLab web page.

2. For the script to automatically fork a repository from another account, it first installs an add-in to Git:
...
3. The forked repository created from a previous run needs to be deleted from GitHub/GitLab, manually:
...
upstream git://github.com/hotwilson/some-repo.git (push)
   ...
   Go manually check on your GitHub account at:<br />
...
PROTIP: We recommend that you make an account or project <strong>container</strong> folder to hold the various repositories. This is because git clone commands create the repository folder but not the account folder as well.

We would make a directory if we use <tt>git init</tt> to create the repository.


   ### Clone in account folder

2. Be in that folder when you clone a repository from GitHub. 
   ...
   * To obtain the URL from a browser at your repository, by clicking the green "Clone or Download": 
   ...
   <pre><strong>git clone https://github.com/hotwilson/some-repo
   </strong></pre>

   If our account is protected by two-factor authenication, you need to use the <strong>git@</strong> URL format with the colon before your account name.

   <pre><strong>git clone git@github.com:hotwilson/some-repo --depth=1
   </strong></pre>
...
3. Remember to cd into the repository folder:

   <pre><strong>cd some-repo
   </strong></pre>

   PROTIP: Forgetting to cd into a newly cloned folder is a common mistake.
4. For a listing of repository files and folders in the <strong>Git working directory</strong> for the current <strong>git checkout</strong>, which is "master" when first initialized:
...
6. A <strong>git branch</strong> command shows us the branches defined in the repository (in a read-only operation):
...
2. Compare with another git branch command.
...
3. The script makes changes by using <strong>echo</strong> commands:

   <pre>echo  "hello"  >>newfile.md</pre>
...
4. cat .gitignore to view contents
...
5. To detail the status of changes to the repo:
 ...
 M .gitignore
   ...
   <a name="Add2Staging"></a>

## 6.x Local edit, add & commit
...
2. You can change several files, but only the files you add to <strong>Git's staging area</strong> 
...
4. The commit supplies a message describing changes, which applies to all files added. 
...
5. List history of actions that have occurred on the local machine.
...
6. List <strong>commits</strong> that have occurred on the local machine:
...
9. When a commit is known good as the one to release, 
...
10. Tags require an additional git push command to be pushed to GitHub.
