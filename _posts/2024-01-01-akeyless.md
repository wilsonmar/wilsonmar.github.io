...
A key differentiation of Akeyless is that, unlike all others who depend on cryptography to hide plain text secrets, Akeyless <strong>segments</strong> each secret such that reassembly only occurs at the customer client, not on Akeyless servers. Akeyless calls that its patented "Distributed Fragments Cryptography™ (DFC)" approach. The approach obsoletes the hassle of maintaining customer keys. 
