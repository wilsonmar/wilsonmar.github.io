...
A key differentiation of Akeyless is that, unlike all others who depend on the "strength" of cryptographic algorithms to hide plain text secrets, Akeyless <strong>segments</strong> each secret such that reassembly only occurs at the customer client, not on Akeyless servers. This greatly lowers the potential blast radius of misappropriated credentials. Akeyless calls that its patented "Distributed Fragments Cryptography™ (DFC)". It also obsoletes the hassle of maintaining customer keys.
...
To guard against AZ, Region, and Provider failure, the Akeyless SaaS platform replicated at the Availability Zone and Region levels within all three major cloud providers (AWS, Azure, Google) where their customers are located. This means that if a customer is located in the US, Akeyless will replicate their data in the US, and if they are located in Europe, Akeyless will replicate their data in Europe. This ensures that customers are not charged for cross-region data egress and that their data is available in case of a disaster.
