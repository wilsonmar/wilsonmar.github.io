...
First, we use my automation and manual instructions to set up components as illustrated in this diagram: <a href="#Admin">A</a>, <a href="#AkeylessParent">B</a>, <a href="#LaptopSetup">C</a>, <a href="#GitHub">D</a>, <a href="#ClientApp">E</a>, <a href="#Gateways">F</a>, <a href="#Bastions">G</a>.
...
then processing steps <a href="#(1)">(1)</a>, <a href="#(2)">(2)</a>, etc. 

 use the Akeyless SaaS solution to safely retrieve static secrets from the enterprise-scale HA multi-cloud SaaS AKeyless, automatically from various programming languages.
...
A key differentiation of Akeyless is, unlike all others who depend on the "strength" of cryptographic algorithms to hide plain text secrets, Akeyless <strong>segments</strong> each secret such that reassembly only occurs within the CPU of customer clients, not on Akeyless servers. This greatly lowers the potential blast radius of misappropriated credentials. Akeyless calls that its patented "Distributed Fragments Cryptography™ (DFC)". It makes for data residence sovereignty. It also obsoletes the hassle of maintaining customer keys.

PROTIP: Most importantly, Akeyless provides an answer to eventual <a target="_blank" href="https://wilsonmar.github.io/quantum/">quantum supremacy</a> that can quickly break cryptographic material.

Akeyless provides a <strong>multi-cloud</strong> solution free of cross-region data egress charges.
By contrast, vaults within Azure, AWS, GCP are set up for a <strong>specific region</strong>. This means charges for data egress accrue and administrators need to set up vaults in several regions, with cross-region replication to ensure that data is available in case of a disaster.

To guard against AZ, Region, and Provider failure, the Akeyless SaaS platform replicated at the Availability Zone and Region levels within all three major cloud providers (AWS, Azure, Google) where their customers are located. This means that if a customer is located in the US, Akeyless will replicate their data in the US, and if they are located in Europe, Akeyless will replicate their data in Europe. This ensures that customers are not charged for cross-region data egress and that their data is available in case of a disaster. [<a target="_blank" href="https://www.information-age.com/why-multi-cloud-key-management-is-future-of-enterprise-security-123504002/">BLOG: Why multi-cloud key management is the future of enterprise security</a>]

PROTIP: Unlike HashiCorp Vault cloud PaaS, administrators don't have to specify a server size processing secrets, which requires constant monitoring for adjustment over time. As a true SaaS solution, technicians within AKeyless take care of scaling.


## AKeyless websites

https://www.youtube.com/@Akeyless
...
* Kotlin (Android): <a target="_blank" href="https://github.com/ajithvgiri/stopwatch">Stopwatch</a> and <a target="_blank" href="https://github.com/carlosgub/JetCoMovie-App-">JetCoMovie</a>
